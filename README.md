...
directories.  If it is used, the directory `/opt/esdata` must be created on __all__ Docker Engine hosts prior to
...
application data external to a container.  However, for the Wordpress service, there is a caveat.  Using this configuration, the Wordpress service
...
as a constraint in starting the Wordpress service must be assigned to only one Docker Engine.  This, of course, would not be done in a production environment.  It's used
...
1. Topbeat can then be started using the following command: ```./topbeat -e -c topbeat.yml``` *As described above, do not start this process until Elasticsearch and Kibana have been initialized.  Doing so will create problems in the Topbeat mapping.*
